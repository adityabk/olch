...
class Mage_Catalog_ProductController extends Mage_Core_Controller_Front_Action
{
    ...
    public function sortajaxAction()
    {
        //print_r($_POST);
        
    //echo $_POST['color'].'<->'.$_POST['product'].'<->'.$_POST['matses'].'<br>';
...
    */
    ...
    /*$_productCollection = Mage::getModel('catalog/product')->getCollection()
          ...
          ->addAttributeToFilter('visibility', 4)
          ->addAttributeToFilter('status', 1)                
            ->addAttributeToFilter('material_season', 132)
          ->load();*/

        
    $this->loadLayout();
    $this->getLayout()->getBlock('root')->setTemplate('catalog/product/ajaxlist.phtml');
    $this->renderLayout();
/*$block = $this->getLayout()->createBlock('catalog/product/listhome.phtml')->setProductCollection($_productCollection);
...
*/
       ...
       // $this->getLayout()->createBlock('catalog/product_list')->setTemplate('catalog/product/list.phtml')->toHtml();
        ...
        /*foreach($_productCollection as $key => $value){
   ...
   /* public function productliteAction()
    {
        $this->loadLayout();
        $this->getLayout()->getBlock('root')->setTemplate('catalog/product/media2.phtml');
        $this->renderLayout();
    */
